...
[![Medium](../../Miscellaneous/Badges/Med.svg)](https://leetcode.com/problems/find-minimum-time-to-reach-last-room-ii/)  
...
| [Minimum Time to Visit a Cell In a Grid](https://leetcode.com/problems/minimum-time-to-visit-a-cell-in-a-grid/) | ![Hard](../../Miscellaneous/Badges/Hard.svg) |
...
# Editorial - 3342. Find Minimum Time to Reach Last Room II
...
int minTimeToReach(int **moveTime, int moveTimeSize, int *moveTimeColSize) {
    ...
    while (!isEmpty(q)) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int dist =
                fmax(d[s.x][s.y], moveTime[nx][ny]) + (s.x + s.y) % 2 + 1;
            if (d[nx][ny] > dist) {
                d[nx][ny] = dist;
                e.x = nx, e.y = ny, e.dist = dist;
                enQueue(q, &e);
            }
        }
    }
